...
int vastaus;
/* NAPPI on Arduino pin 9:ssa */
#define NAPPI 2
/* lämpöanturi(SENSOR) on ARDUINO pinni A0:ssa */
#define SENSOR A0
...
void keskeytys() {
  //* DEBUGGAUSTA VARTEN */
  Serial.println("Keskeytys tapahtui!");
  /* sijoita lampoanturin arvo muuttujaan (adc) */
  int adc = analogRead(SENSOR);
  /* Kutsutaan muunnaADCtoLampotila()-funktiota 
   * ja sijoitetaan sen palauttama arvo
   * lampotila-muuttujaan
   */
  float lampotila = muunnaADCtoLampotila(adc);
...
float muunnaADCtoLampotila(int adcArvo) {
  /* Laskee lampöanturin arvon
   * celsiuseksi
   * ja palauttaa arvon
   */
  return 0.7 * adcArvo - 77.4;
}
